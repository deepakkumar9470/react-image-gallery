...
const Modal = ({ selectedImage, close, darkMode }) => {
  return (
          ...
          <div className="flex justify-between items-center absolute bottom-[28%]">
            <div className="flex items-center gap-2 px-2">
              ...
              <div className="bg-transparent text-white rounded-sm  border px-2 py-1 cursor-pointer flex items-center gap-1">
                ...
                <span className="text-[8px] font-poppins capitalize">
                  {selectedImage?.user.social.twitter_username}Info
                </span>
              </div>
            </div>
            ...
            <div>
              <ArrowDownToLine
                className="w-6 h-6 p-1 bg-[#3CB46E] flex flex-end text-white rounded-full cursor-pointer"
                size={14}
                strokeWidth={1.5}
              >
                <a
                  className="text-sm"
                  href={selectedImage?.links.download_location}
                  download
                >
                  Download Image
                </a>
    ...
    text-[8px] text-sm font-semibold font-montserrat ${
      darkMode ? "text-[#A7A7A7]" : "text-[#000]"
          ...
          <div className="flex flex-col gap-4 px-4 mb-2">
            <p
              className={`text-[12px] font-semibold font-montserrat ${
                darkMode ? "text-black" : "text-[#ECECEC]"
              }`}
            >
              Related Tags
            </p>
            ...
            <div className="flex gap-4">
              {selectedImage?.tags?.map((t, i) => (
                <span
                  key={i}
                  className="text-[10px] font-semibold bg-[#ECECEC] text-black rounded-md p-1 font-montserrat"
                >
                  {t.title}
                </span>
              ))}
